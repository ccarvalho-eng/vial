defmodule Vial.Providers do
  @moduledoc """
  Context for managing AI provider configurations.

  In embedded mode, all functions accept a repo as the first parameter.
  For standalone mode, you can pass Vial.Repo directly.
  """

  alias Vial.Providers.Provider

  @doc """
  Lists all providers in the system.
  """
  @spec list_providers(module()) :: [Provider.t()]
  def list_providers(repo) do
    repo.all(Provider)
  end

  @doc """
  Gets a provider by ID, raising if not found.
  """
  @spec get_provider!(module(), binary()) :: Provider.t()
  def get_provider!(repo, id) do
    repo.get!(Provider, id)
  end

  @doc """
  Creates a new provider.
  """
  @spec create_provider(module(), map()) ::
          {:ok, Provider.t()} | {:error, Ecto.Changeset.t()}
  def create_provider(repo, attrs \\ %{}) do
    %Provider{}
    |> Provider.changeset(attrs)
    |> repo.insert()
  end

  @doc """
  Updates an existing provider.
  """
  @spec update_provider(module(), Provider.t(), map()) ::
          {:ok, Provider.t()} | {:error, Ecto.Changeset.t()}
  def update_provider(repo, %Provider{} = provider, attrs) do
    provider
    |> Provider.changeset(attrs)
    |> repo.update()
  end

  @doc """
  Deletes a provider.
  """
  @spec delete_provider(module(), Provider.t()) ::
          {:ok, Provider.t()} | {:error, Ecto.Changeset.t()}
  def delete_provider(repo, %Provider{} = provider) do
    repo.delete(provider)
  end

  @doc """
  Returns a changeset for tracking provider changes.
  """
  @spec change_provider(Provider.t(), map()) :: Ecto.Changeset.t()
  def change_provider(%Provider{} = provider, attrs \\ %{}) do
    Provider.changeset(provider, attrs)
  end
end
