...
defmodule Ecto.Query.Builder.CTE do
  ...
  def build(query, name, cte, materialized, operation, env) do
    Builder.apply_query(
      query,
      __MODULE__,
      [escape(name, env), build_cte(name, cte, env), materialized, operation],
      env
    )
  end
  ...
  def apply(
        %Ecto.Query{with_ctes: with_expr} = query,
        name,
        %_{} = with_query,
        materialized,
        operation
  ...
  def apply(%Ecto.Query{with_ctes: with_expr} = query, name, with_query, materialized, operation) do
    update =
      quote do
        Ecto.Query.Builder.CTE.apply_cte(
          unquote(with_expr),
          unquote(name),
          unquote(with_query),
          unquote(materialized),
          unquote(operation)
        )
      end
  ...
  def apply_cte(nil, name, with_query, materialized, operation) when is_boolean(materialized) do
    %Ecto.Query.WithExpr{
      queries: [{name, %{materialized: materialized, operation: operation}, with_query}]
    }
  end
  ...
  def apply_cte(
        %Ecto.Query.WithExpr{queries: queries} = with_expr,
        name,
        with_query,
        materialized,
        operation
    ...
    %{
      ...
      | queries:
          List.keystore(
            ...
            name,
            0,
            {name, %{materialized: materialized, operation: operation}, with_query}
          )
    }
  end
  ...
  def apply_cte(
    ...
    %{
      with_expr
      | queries: List.keystore(queries, name, 0, {name, %{operation: operation}, with_query})
    }
  end
end
