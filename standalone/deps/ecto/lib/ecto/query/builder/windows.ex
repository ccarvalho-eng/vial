...
defmodule Ecto.Query.Builder.Windows do
  ...
  def runtime!(query, runtime, file, line) do
    windows =
      Enum.map(runtime, fn {name, compile_acc, runtime_acc, params, escape_acc} ->
        {{acc, subqueries}, params} =
          do_runtime_window!(runtime_acc, query, {compile_acc, escape_acc.subqueries}, params)
  ...
  defp do_runtime_window!([{:order_by, order_by} | kw], query, {acc, subqueries_acc}, params) do
    {order_by, params, subqueries} =
      OrderBy.order_by_or_distinct!(:order_by, query, order_by, params)

    do_runtime_window!(
      kw,
      query,
      {[{:order_by, order_by} | acc], subqueries_acc ++ subqueries},
      params
    )
  end
  ...
  defp do_runtime_window!(
    ...
    {partition_by, params, subqueries} =
      GroupBy.group_or_partition_by!(:partition_by, query, partition_by, params)

    do_runtime_window!(
      kw,
      query,
      {[{:partition_by, partition_by} | acc], subqueries_acc ++ subqueries},
      params
    )
  end
