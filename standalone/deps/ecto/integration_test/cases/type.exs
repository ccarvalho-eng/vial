defmodule Ecto.Integration.TypeTest do
  ...
  test "bitstring type" do
    ...
    # Bitstrings
    assert [^bitstring] =
             TestRepo.all(from p in Bitstring, where: p.bs == ^bitstring, select: p.bs)
  ...
  test "tagged types" do
    ...
    # parent_as/1
    child =
      from c in Comment, where: type(parent_as(:posts).id, :string) == c.text, select: c.post_id
  ...
  test "json_extract_path with primitive values" do
    ...
    assert TestRepo.one(from o in Order, where: o.metadata["enabled"] == true, select: o.id) ==
             order.id

    assert TestRepo.one(
             from o in Order, where: o.metadata["extra"][0]["enabled"] == false, select: o.id
           ) == order.id
  end
