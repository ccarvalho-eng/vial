defmodule Ecto.Integration.SubQueryTest do
  ...
  test "subqueries in order by" do
    ...
    query =
      from p in Post,
        as: :p,
        order_by: [asc: exists(from p in Post, where: p.visits > parent_as(:p).visits)]
  ...
  test "subqueries in distinct" do
    ...
    query =
      from p in Post,
        as: :p,
        distinct: exists(from p in Post, where: p.visits > parent_as(:p).visits),
        order_by: [asc: :title]
  ...
  test "subqueries in group by" do
    ...
    query =
      from p in Post,
        as: :p,
        select: sum(p.visits),
        group_by: exists(from p in Post, where: p.visits > parent_as(:p).visits),
        order_by: [sum(p.visits)]
