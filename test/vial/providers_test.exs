defmodule Vial.ProvidersTest do
  ...
  describe "providers" do
    test "list_providers/0 returns all providers" do
      ...
      assert Providers.list_providers(Repo) == [provider]
    end

    test "get_provider!/1 returns the provider with given id" do
      provider = provider_fixture()
      assert Providers.get_provider!(Repo, provider.id) == provider
    end
    ...
    test "create_provider/1 with valid data creates a provider" do
      ...
      assert {:ok, provider} = Providers.create_provider(Repo, attrs)
    ...
    test "create_provider/1 with invalid provider enum returns error" do
      ...
      assert {:error, changeset} = Providers.create_provider(Repo, attrs)
    ...
    test "update_provider/2 with valid data updates the provider" do
      ...
      assert {:ok, updated} = Providers.update_provider(Repo, provider, update_attrs)
    ...
    test "update_provider/2 with invalid data returns error" do
      provider = provider_fixture()
      assert {:error, changeset} = Providers.update_provider(Repo, provider, %{name: nil})
      assert "can't be blank" in errors_on(changeset).name
    end

    test "delete_provider/1 deletes the provider" do
      provider = provider_fixture()
      assert {:ok, _} = Providers.delete_provider(Repo, provider)
      assert_raise Ecto.NoResultsError, fn -> Providers.get_provider!(Repo, provider.id) end
    end
